...
function flowerpot.register_node(nodename)
	...
	minetest.register_node("flowerpot:" .. nodedef.name:gsub(":", "_"), {
		description = "Flowerpot with " .. nodedef.description,
		drawtype = "mesh",
		mesh = "flowerpot.obj",
		tiles = tiles,
		paramtype = "light",
		...
		drop = {
			items = {
				{items = {
					"flowerpot:empty",
					nodename:gsub("grass_%d", "grass_1") -- plant in the pot
				}}
			}
		},
...
for node, def in pairs(minetest.registered_nodes) do
	if def.groups.flora == 1 or def.groups.sapling == 1 or def.groups.plant == 1
	or node:find("mushroom") or string.sub(node, -5) == "_stem" then
		nodes[#nodes+1] = node
	end
end
